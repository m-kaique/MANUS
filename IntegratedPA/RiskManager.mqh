...
#include "Logger.mqh"
#include "Volatility_Adjuster.mqh"
...
private:
   ...
   CVolatilityAdjuster *m_volatilityAdjuster;
   CVolatilityAdjuster *m_volatilityAdjusters[];
...
public:
   ...
   bool Initialize(CLogger *logger, CMarketContext *marketContext, CCircuitBreaker *circuitBreaker);
   ...
   bool ValidateStopLoss(string symbol, double price, double stopLoss, ENUM_ORDER_TYPE direction, string &errorMsg);
   double GetScalingFactorByQuality(SETUP_QUALITY quality);
...
{
   if (m_volatilityAdjuster != NULL)
      delete m_volatilityAdjuster;
}
...
bool CRiskManager::ValidateLargeLotPartials(string symbol, double totalVolume, double &partialPercentages[], int numPartials)
{
   ...
   if (minLot <= 0)
      minLot = 100.0; // Padrão para ações
   ...
   if (m_partialMetrics.totalOperations > 0)
   {
      m_partialMetrics.avgVolumeIncrease =
          ((m_partialMetrics.totalVolumeFinal - m_partialMetrics.totalVolumeOriginal) /
           m_partialMetrics.totalVolumeOriginal) *
          100.0;
...
OrderRequest CRiskManager::BuildRequest(string symbol, Signal &signal, MARKET_PHASE phase)
{
   ...
   // [Circuit Breaker] Checagem antifalhas ANTES de tudo
   if (m_circuitBreaker != NULL && !m_circuitBreaker.IsOperationAllowed("BuildRequest"))
   {
      if (m_logger != NULL)
         m_logger.Warning("BuildRequest bloqueada pelo Circuit Breaker");
      request.volume = 0;
      return request;
   }
   ...
   // [1] Validação de símbolo
   if (symbol == "" || StringLen(symbol) == 0)
   {
      if (m_logger != NULL)
         m_logger.Error("RiskManager: Símbolo inválido para construção de requisição");
      if (m_circuitBreaker != NULL)
         m_circuitBreaker.RegisterError("Símbolo inválido");
      request.volume = 0;
      return request;
   }

   // [2] Validação de preço de mercado
   ...
   if (!ValidateMarketPrice(symbol, marketPrice))
   {
      ...
      if (m_circuitBreaker != NULL)
         m_circuitBreaker.RegisterError("Preço de mercado inválido");
      request.volume = 0;
      return request;
   }
   ...
   if (!SymbolInfoTick(symbol, lastTick))
   {
      if (m_logger != NULL)
         m_logger.Error("RiskManager: Falha ao obter tick para " + symbol);
      if (m_circuitBreaker != NULL)
         m_circuitBreaker.RegisterError("Erro tick");
      request.volume = 0;
      return request;
   }
   ...
   if (priceDeviation > maxDeviation)
   {
      ...
      if (m_logger != NULL)
         m_logger.Info(StringFormat("RiskManager: Preço ajustado de %.5f para %.5f", signal.entryPrice, request.price));
   }

   // [3] Validação robusta de stop loss
   string slError;
   if (!ValidateStopLoss(symbol, request.price, request.stopLoss, signal.direction, slError))
   {
      m_logger.Error("SL inválido: " + slError);
      if (m_circuitBreaker != NULL)
         m_circuitBreaker.RegisterError("Stop Loss inválido");
      request.volume = 0;
      return request;
   }

   // [4] Obtenha índice do símbolo (garante m_symbolParams e adjuster alinhados)
   int index = FindSymbolIndex(symbol);
   double riskPercentage = (index >= 0) ? m_symbolParams[index].riskPercentage : m_defaultRiskPercentage;

   // [5] Cálculo de volume base (antes de ajustes)
   ...
   if (baseVolume <= 0)
   {
      ...
      if (m_circuitBreaker != NULL)
         m_circuitBreaker.RegisterError("Volume inválido");
      request.volume = 0;
      return request;
   }

   // [6] Escalamento granular por qualidade do setup
   double scalingFactor = GetScalingFactorByQuality(signal.quality);
   baseVolume *= scalingFactor;
   if (m_logger != NULL)
      m_logger.Info(StringFormat("Escalamento granular [%s]: %.1fx → baseVolume %.3f lotes", EnumToString(signal.quality), scalingFactor, baseVolume));

   // [7] AJUSTE POR VOLATILIDADE MULTI-SÍMBOLO
   // Checa se há adjuster para o símbolo (index é sempre igual em m_volatilityAdjusters e m_symbolParams)
   if (index >= 0 && ArraySize(m_volatilityAdjusters) > index && m_volatilityAdjusters[index] != NULL)
   {
      double ajusteVol = m_volatilityAdjusters[index].GetAdjustmentFactor();
      baseVolume *= ajusteVol;
      if (m_logger != NULL)
   ...
   if (index >= 0 && m_symbolParams[index].usePartials)
   {
      ...
      if (request.volume > MAX_VOLUME_PER_TRADE)
      {
         if (m_logger != NULL)
            m_logger.Warning(StringFormat("Volume limitado: %.2f → %.2f lotes (limite de segurança)", request.volume, MAX_VOLUME_PER_TRADE));
         request.volume = MAX_VOLUME_PER_TRADE;
      }
   ...
   if (index >= 0 && m_symbolParams[index].maxLotSize > 0)
   {
      ...
      {
         if (m_logger != NULL)
            m_logger.Warning(StringFormat("Volume limitado para %s: %.2f → %.2f lotes (máximo configurado)", symbol, request.volume, m_symbolParams[index].maxLotSize));
         request.volume = m_symbolParams[index].maxLotSize;
      }
   }

   // [10] Ajuste final para lote válido
   ...
   if (request.volume <= 0)
   {
      ...
      if (m_circuitBreaker != NULL)
         m_circuitBreaker.RegisterError("Volume final inválido");
      return request;
   }

   // [12] Dados finais e sucesso
   request.signalId = signal.id;
   request.id = (int)GetTickCount();
   if (m_logger != NULL)
      m_logger.Info(StringFormat("✅ REQUISIÇÃO CRIADA para %s: %.3f lotes, preço %.5f, SL %.5f, TP %.5f", symbol, request.volume, request.price, request.stopLoss, request.takeProfit));
   if (m_circuitBreaker != NULL)
...
//+------------------------------------------------------------------+
//| Validação robusta de Stop Loss                          a     |
//+------------------------------------------------------------------+
bool CRiskManager::ValidateStopLoss(string symbol,
                                    double price,
                                    double stopLoss,
                                    ...
                                    string &errorMsg)
{
   ...
   if (stopLoss <= 0)
   {
      errorMsg = "Stop loss deve ser maior que zero";
      return false;
   }

   // 2. Obter dados do símbolo
   double point = SymbolInfoDouble(symbol, SYMBOL_POINT);
   ...
   // 4. Validação direcional
   if (direction == ORDER_TYPE_BUY)
   {
      if (stopLoss >= price)
      {
         errorMsg = StringFormat("SL (%.5f) deve estar abaixo do preço (%.5f) para compra", stopLoss, price);
         return false;
      }
   }
   else if (direction == ORDER_TYPE_SELL)
   {
      if (stopLoss <= price)
      {
         errorMsg = StringFormat("SL (%.5f) deve estar acima do preço (%.5f) para venda", stopLoss, price);
         return false;
      }
   }
   ...
   {
      errorMsg = StringFormat("Distância SL (%.1f pts) menor que mínimo permitido (%.1f pts)", distance / point, minStopLevel / point);
      return false;
   }

   // 6. Distância máxima (proteção contra erros)
   double maxDistance = price * 0.10; // 10% do preço
   if (distance > maxDistance)
   {
      errorMsg = StringFormat("Distância SL (%.1f pts) excessiva (máx: %.1f pts)", distance / point, maxDistance / point);
      return false;
   }

   // 7. Spread vs SL
   if (spread > distance * 0.5)
   {
      errorMsg = StringFormat("Spread (%.1f pts) muito alto comparado ao SL (%.1f pts)", spread / point, distance / point);
      return false;
   }
...
double CRiskManager::GetScalingFactorByQuality(SETUP_QUALITY quality)
{
   for (int i = 0; i < ArraySize(g_scalingTable); i++)
   {
      if (g_scalingTable[i].quality == quality)
      {
         return g_scalingTable[i].maxScalingFactor;
      }
   }
   return 1.0;
}
