...
struct ConfluenceFactors {
   ...
   void Calculate() {
      ...
      if(patternQuality) totalFactors++;
      if(maAlignment) totalFactors++;
      if(volumeConfirmation) totalFactors++;
      if(nearKeyLevel) totalFactors++;
      if(strongTrend) totalFactors++;
      if(momentumConfirmed) totalFactors++;
      if(mtfConfirmation) totalFactors++;
      if(favorableStructure) totalFactors++;
      if(optimalSession) totalFactors++;
      if(goodRiskReward) totalFactors++;
...
bool CSetupClassifier::CheckMAAlignment(string symbol, ENUM_TIMEFRAMES timeframe, Signal &signal) {
   if(m_marketContext == NULL) return false;
...
bool CSetupClassifier::CheckNearKeyLevel(string symbol, ENUM_TIMEFRAMES timeframe, double price) {
   ...
   if(nearestLevel <= 0) return false;
...
bool CSetupClassifier::CheckMultiTimeframeConfirmation(string symbol, ENUM_TIMEFRAMES timeframe, Signal &signal) {
   ...
   if(CopyBuffer(ema21Handle, 0, 0, 1, ema21) > 0 && 
      ...
      if(ema21[0] > ema50[0]) higherTrend = 1;
      else if(ema21[0] < ema50[0]) higherTrend = -1;
   }
   ...
   if(signal.direction == ORDER_TYPE_BUY && higherTrend > 0) return true;
   if(signal.direction == ORDER_TYPE_SELL && higherTrend < 0) return true;
...
double CSetupClassifier::FindNearestKeyLevel(string symbol, ENUM_TIMEFRAMES timeframe, double price) {
   if(m_marketContext == NULL) return 0;
...
bool CSetupClassifier::IsWithinSpreadLimit(string symbol, double currentSpread) {
   ...
   if(count == 0) return true; // Se não há dados, permitir
